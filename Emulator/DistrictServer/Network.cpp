...
int Network::SendInitial(int districtType, int districtId, char* address, char* port, char* token)
{
	char data[255];
	int a = 0;
	if (strlen(address) < 10) a = 1;
	else a = 2;
	sprintf_s(data, sizeof(data), "%d%d%d%d%d%s%d%d%s%d%d%s", 0, districtType, districtId, a, strlen(address), address, 0, strlen(port), port, 0, 8, token);
	return Send(data);
}
